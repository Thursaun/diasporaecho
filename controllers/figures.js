...
const removeSavedFigure = (req, res, next) => {
  const userId = req.user._id;
  const figureId = req.params.figureId;
  ...
  User.findById(userId)
    .then((user) => {
      user.savedFigures = user.savedFigures.filter(
        (id) => id.toString() !== figureId
      );
      return user.save();
    })
    .then(() => {
      return Figure.findById(figureId);
    })
    .then((figure) => {
      if (figure) {
        ...
        if (figure.owners.length === 0) {
          return Figure.findByIdAndDelete(figureId).then(() => ({
            message: "Figure removed completely",
          }));
        } else {
          return figure
            .save()
            .then(() => ({ message: "Figure removed from saved list" }));
        }
      }
      return { message: "Figure not found" };
    })
    .then((result) => {
