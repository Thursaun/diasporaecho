      ...
      >
        ...
        >
          ...
          <div className="relative z-10 text-center mb-2">
            ...
            <span className="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-white/90 text-gray-800 shadow-sm mb-2">
              {categories[0] || occupation[0] || "Historical Figure"}
            </span>
            {displayYears && (
              <p className="text-sm text-gray-300 font-medium">{displayYears}</p>
            )}
          </div>

          {/* Achievements List - Positioned below header with ~2 line gap */}
          <div className="relative z-10 mt-4 overflow-hidden">
            <div className="flex items-center justify-center gap-2 mb-3 pb-2 border-b border-amber-400/50">
              <svg className="w-4 h-4 text-amber-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
              <span className="text-sm font-semibold text-amber-400 uppercase tracking-wide">Key Achievements</span>
            </div>
            
            <ul className="space-y-2 w-full text-center">
              {(() => {
                // BUILD ACHIEVEMENTS FROM WIKIDATA METADATA FIRST
                const achievements = [];
                
                // Helper to capitalize first letter
                const capitalize = (str) => str.charAt(0).toUpperCase() + str.slice(1);
                
                // Add awards (most prestigious)
                if (awards && awards.length > 0) {
                  awards.slice(0, 2).forEach(award => {
                    achievements.push({ icon: '🏆', text: capitalize(award) });
                  });
                }
                
                // Add notable works
                if (notableWorks && notableWorks.length > 0) {
                  notableWorks.slice(0, 2).forEach(work => {
                    achievements.push({ icon: '📚', text: `"${capitalize(work)}"` });
                  });
                }
                
                // Add education
                if (education && education.length > 0) {
                  education.slice(0, 1).forEach(school => {
                    achievements.push({ icon: '🎓', text: `Studied at ${capitalize(school)}` });
                  });
                }
                
                // Add movement (don't add "movement" if it's already in the text)
                if (movement && movement.length > 0) {
                  movement.slice(0, 1).forEach(m => {
                    const movementText = m.toLowerCase().includes('movement') 
                      ? capitalize(m)
                      : `${capitalize(m)} movement`;
                    achievements.push({ icon: '✊', text: movementText });
                  });
                }
                
                // Add birthplace
                if (birthPlace) {
                  achievements.push({ icon: '📍', text: `Born in ${capitalize(birthPlace)}` });
                }
                
                // If we have metadata achievements, use them (max 4 with 2-line clamp)
                if (achievements.length >= 2) {
                  return achievements.slice(0, 4).map((item, i) => (
                ...
                // FALLBACK: Extract from description if no metadata
                ...
                if (sentences.length >= 2) {
                  return sentences.map((sentence, i) => (
                    <li key={i} className="flex items-center justify-center gap-2">
                      <span className="text-secondary flex-shrink-0 text-sm">•</span>
                      <span className="text-xs sm:text-sm text-gray-300 leading-snug">
                        {sentence.replace(/[.!?]$/, '')}
                      </span>
                    </li>
                ...
                // Ultimate fallback: split description into chunks
                const words = description.split(' ');
                const chunkSize = Math.ceil(words.length / 3);
                const chunks = [];
                for (let i = 0; i < words.length && chunks.length < 3; i += chunkSize) {
          ...
          {/* Footer with flip back button - stays at bottom */}
          <div className="relative z-10 flex justify-center mt-auto pt-3 border-t border-white/10">
            <button
              onClick={handleFlipBack}
              className="px-4 py-2 bg-white/10 backdrop-blur-sm text-white text-xs sm:text-sm font-medium rounded-lg hover:bg-white/20 transition-all duration-200 flex items-center gap-2 border border-white/20"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
